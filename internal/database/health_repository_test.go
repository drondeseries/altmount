...
func TestGetFilesForLibrarySync(t *testing.T) {
	repo := setupTestDB(t)
	ctx := context.Background()

	// 1. File with NULL library_path
	_, err := repo.db.ExecContext(ctx, `
	...
	// 2. File with library_path in mount
	_, err = repo.db.ExecContext(ctx, `
		INSERT INTO file_health (file_path, library_path, status)
		VALUES (?, ?, ?)
	`, "mount_path.mkv", "/mnt/mount/some/file.mkv", "healthy")
	require.NoError(t, err)

	// 3. File with library_path in library dir
	_, err = repo.db.ExecContext(ctx, `
		INSERT INTO file_health (file_path, library_path, status)
		VALUES (?, ?, ?)
	...
	files, err := repo.GetFilesForLibrarySync(ctx)
	require.NoError(t, err)
