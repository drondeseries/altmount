...
func (s *Server) handleArrsWebhook(c *fiber.Ctx) error {
	...
	for _, path := range filesToDelete {
		...
		if s.configManager != nil && metadataPath != "" && metadataPath != "." && metadataPath != "/" {
			...
			if cfg.MountPath != "" {
				...
				// Additional hard protection for user's specific symlink
				if cleanLocal == "/mnt/usenet-rclone" || cleanLocal == "/mnt/altmount-rclone" {
					slog.WarnContext(c.Context(), "Nuclear Guard: Blocked attempt to delete protected mount point", "path", cleanLocal)
					continue
				}
	...
	for _, path := range pathsToScan {
		...
		if s.healthRepo != nil {
			...
			if req.DownloadId != "" && sourceNzb != nil && s.queueRepo != nil {
				...
				_ = s.queueRepo.UpsertImportHistory(c.Context(), &database.ImportHistory{
					DownloadID:  &req.DownloadId,
					NzbName:     nzbName,
					FileName:    filepath.Base(normalizedPath),
