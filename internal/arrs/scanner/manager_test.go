...
func TestFindInstanceForFilePath_CategoryMatch(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "completeDir set to /, matches radarr by category moviesHQ (FUSE path style)",
			completeDir: "/",
			filePath:    "/moviesHQ/The.Devil.Wears.Prada.2006.2160p/The.Devil...mkv",
			radarrCat:   "moviesHQ",
			sonarrCat:   "tvHQ",
			wantType:    "radarr",
			wantName:    "radarr_instance",
		...
		{
			name:        "completeDir set to /, matches radarr by category moviesHQ (import path style)",
			completeDir: "/",
			filePath:    "/tmp/altmount-uploads/moviesHQ/The.Firm.1993/The.Firm.mkv",
			radarrCat:   "moviesHQ",
			sonarrCat:   "tvHQ",
			wantType:    "radarr",
			wantName:    "radarr_instance",
		...
		{
			name:        "completeDir set to /, matches sonarr by category tvHQ (relative path style)",
			completeDir: "/",
			filePath:    "tvHQ/Abbott.Elementary/Abbott...mkv",
			radarrCat:   "moviesHQ",
			sonarrCat:   "tvHQ",
			wantType:    "sonarr",
			wantName:    "sonarr_instance",
