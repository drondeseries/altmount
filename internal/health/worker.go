...
type ARRsRepairService interface {
	...
	DiscoverFileMetadata(ctx context.Context, filePath, relativePath, nzbName string) (*model.WebhookMetadata, error)
}
	...
	if event.Type == EventTypeFileHealthy {
		...
		sideEffect = func() error {
			...
			if fh.Metadata == nil || *fh.Metadata == "" {
				...
				metadata, err := hw.arrsService.DiscoverFileMetadata(ctx, fh.FilePath, relativePath, nzbName)
...
func (hw *HealthWorker) ensureMetadata(ctx context.Context, item *database.FileHealth) *string {
	...
	metadata, err := hw.arrsService.DiscoverFileMetadata(ctx, item.FilePath, relativePath, nzbName)
	if err == nil && metadata != nil {
		metaBytes, err := json.Marshal(metadata)
		if err == nil {
