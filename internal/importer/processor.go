...
func (proc *Processor) ProcessNzbFile(ctx context.Context, filePath, relativePath string, queueID int, allowedExtensionsOverride *[]string, virtualDirOverride *string, extractedFiles []parser.ExtractedFileInfo, category *string, metadata *string, downloadID *string, instanceName *string) (string, []string, error) {
	cfg := proc.configGetter()
...
func (proc *Processor) processSingleFile(
	...
	if proc.recorder != nil {
		nzbID := int64(queueID)
		if downloadID != nil {
			slog.InfoContext(ctx, "Recording history with DownloadID", "download_id", *downloadID, "nzb_name", nzbName)
		} else {
			slog.WarnContext(ctx, "Recording history with NO DownloadID", "nzb_name", nzbName)
		}
		_ = proc.recorder.UpsertImportHistory(ctx, &database.ImportHistory{
