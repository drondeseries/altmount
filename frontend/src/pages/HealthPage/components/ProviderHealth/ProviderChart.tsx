			...
			<div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
				{sortedPayload.map((p) => (
					<div key={p.dataKey} className="flex items-center justify-between gap-4 py-0.5">
						...
						<span className="font-semibold font-mono text-base-content">{formatBytes(p.value)}</span>
					</div>
				))}
			</div>
			<div className="mt-3 flex justify-between border-base-200/30 border-t pt-2 font-bold text-sm">
				<span className="text-base-content/70">Total:</span>
				<span className="text-info font-mono">{formatBytes(sum)}</span>
			</div>
		</div>
	);
};
			...
			<div className="card-body p-6">
				<div className="mb-6 flex flex-col items-start justify-between gap-4 lg:flex-row lg:items-center">
					<div>
						<h2 className="card-title flex items-center gap-2 text-lg font-bold">
							<BarChart3 className="h-5 w-5 text-info animate-pulse" />
							Data Usage Trends
						</h2>
						<p className="text-base-content/60 text-xs mt-0.5">Total volume: {formatBytes(totalUsage)} in the last {days} days</p>
					</div>
				...
				<div className="flex h-80 w-full flex-col gap-6 lg:flex-row">
					<div className="h-full w-full flex-grow lg:w-3/4">
						<ResponsiveContainer width="100%" height="100%">
							<AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
								...
								<YAxis tick={{ fontSize: 10, fill: "currentColor", opacity: 0.7 }} axisLine={false} tickLine={false} tickFormatter={formatBytes} />
					...
					<div className="flex hidden h-full w-full flex-col items-center justify-center border-base-200/40 border-l pl-4 lg:flex lg:w-1/4">
						<span className="mb-2 font-bold text-base-content/70 text-xs tracking-wider uppercase">Usage Breakdown</span>
						<ResponsiveContainer width="100%" height="100%">
							<PieChart>
								<Pie
									data={pieData}
